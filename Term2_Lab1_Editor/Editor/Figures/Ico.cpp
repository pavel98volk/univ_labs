...
Ico::Ico(GLfloat R) {
	...
	float GOLDEN = (1.0 + sqrt(5.0)) / 2.0 *R;
	this->R = sqrt(R*(1 + GOLDEN*GOLDEN));
	float** vertices = new float*[12];
	for (int i = 0;i < 12;i++) vertices[i] = new float[3];
	for (int i = 0;i < 3;i++)
		for (int j = 0;j < 4;j++) {
			vertices[i * 4 + j][i] = (j % 2) ? R : -R;
			vertices[i * 4 + j][(i + 1) % 3] = (j < 2) ? GOLDEN : -GOLDEN;
...
void Ico::refine() {
    ...
    for(GLfloat** a: Faces) {
		...
		(*Faces_next).push_back(initFace(a[0], a[1], a[2]));
...
void Ico::draw() {
   ...
   float color=0;
    for(GLfloat** a: Faces) {
        glColor3f(color*2-int(color*2),1-color+int(color),0.3+color-int(color));
        color+=0.23;
		x[0] = a[0][0] + a[1][0] + a[2][0];
		x[1] = a[0][1] + a[1][1] + a[2][1];
		x[2] = a[0][2] + a[1][2] + a[2][2];
		glNormal3fv(x);
		glVertex3fv(a[0]);
		glVertex3fv(a[1]);
		glVertex3fv(a[2]);
